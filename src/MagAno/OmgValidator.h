...
public:
    ...
    {
        // -180.0~180.0
        QRegExp rx("^-?180(\\.0+)?|-?1[0-7]\\d(\\.\\d+)?|-?[0-9]?\\d(\\.\\d+)?$");
        QRegExpValidator *pReg = new QRegExpValidator(rx);
        line_edit->setValidator(pReg);
    }

    static void setValidatorLat_mamea(QLineEdit *line_edit)
    {
        // -12.0~46.0
        QRegExp rx("^(-?1[0-2](\.\d+)?|-?\d{1,2}(\.\d+)?|4[0-5](\.\d+)?|46(\.0+)?)$");
        QRegExpValidator *pReg = new QRegExpValidator(rx);
        line_edit->setValidator(pReg);
    }

    static void setValidatorLon_mamea(QLineEdit *line_edit)
    {
        // 93.0~160.0
        QRegExp rx("^(9[3-9](\.\d+)?|1[0-5]\d(\.\d+)?|160(\.0+)?)$");
        QRegExpValidator *pReg = new QRegExpValidator(rx);
        line_edit->setValidator(pReg);
    }
