...
//    char  mdfile[PATH] = "wmm/COF/IGRF13.COF";
//    char mdfile[PATH] = "D:/Qtapp/whumag/wmm/COF/IGRF14_Windows.COF";
    char mdfile[PATH] = "./COF/IGRF14_Windows.COF";
    ...
    for (int index = 0; index < length; ++index)
    {
        ...
        MAGtype_Date UserDate = UserDateArr[index];

        sdate = UserDate.DecimalYear;
...
//        printf("IGRF INPUT: Lat: %f, Lon: %f, Alt: %f, Date: %f\n", latitude, longitude, alt, sdate);
        ...
        GeoMagneticElements.F = f;
        GeoMagneticElements.H = h;
        GeoMagneticElements.X = x;
        GeoMagneticElements.Y = y;
        GeoMagneticElements.Z = z;
        GeoMagneticElements.Decl = d;
        GeoMagneticElements.Incl = i;
        GeoMagneticElements.Fdot = fdot;
        GeoMagneticElements.Hdot = hdot;
        GeoMagneticElements.Xdot = xdot;
        GeoMagneticElements.Ydot = ydot;
        GeoMagneticElements.Zdot = zdot;
        GeoMagneticElements.Decldot = ddot;
        GeoMagneticElements.Incldot = idot;
        GeoMagneticElementsArr[index] = GeoMagneticElements;
    }
...
int main_igrf(int argc, char **argv)
{
#ifdef MAC
    ccommand(argc, argv);
#endif
    ...
    while (again == 1)
    {
        if (coords_from_file)
        {
            ...
            fprintf(outfile, "%s %s %s %s %s ", args[2], args[3], args[4], args[5], args[6]);
            fflush(outfile);
            iline++;
        } /* coords_from_file */
            ...
            else
            {
                ...
                for (syr = sdate + step; syr < (edate + step); syr += step)
                {
                    ...
                    shval3(igdgc, latitude, longitude, alt, nmax, 4,
                           IEXT, EXT_COEFF1, EXT_COEFF2, EXT_COEFF3);
                    dihf(4);
                    printf("IGRF Computed: X: %f, Y: %f, Z: %f, H: %f, F: %f\n", x, y, z, h, f);
